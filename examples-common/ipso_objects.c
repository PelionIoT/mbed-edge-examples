...
pt_object_instance_t* ipso_create_custom_object(pt_device_t *device, enum IPSO_OBJECTS object_id, enum IPSO_RESOURCES resourceId, Lwm2mResourceType type, uint8_t operations, const void* value, const uint32_t value_size, pt_resource_callback callback)
{
    pt_status_t status = PT_STATUS_SUCCESS;
...
pt_object_instance_t* ipso_create_sensor_object(pt_device_t *device, enum IPSO_OBJECTS sensor_id, const char *sensor_units, const char *sensor_type)
{
    ...
    pt_object_t *object = pt_device_add_object(device, sensor_id, &status);
    if (status != PT_STATUS_SUCCESS) {
        tr_err("Could not create an object with id (%d) to the device (%s).",
...
void ipso_reset_min_max_object(const pt_resource_opaque_t *resource, const uint8_t* value, uint32_t value_len, void *userdata)
{
